...
router
  ...
  .get('/callback', (req, res) => {
    ...
    spotifyApi.authorizationCodeGrant(code).then(
      async (data) => {
        ...
        res.redirect(
          `http://localhost:3000/auth?access_token=${data.body['access_token']}&refresh_token=${data.body['refresh_token']}&name=${userData.display_name}`
        );
      },
      (err) => {
  ...
  .get('/createplaylist', (req, res) => {
    ...
    spotifyApi.setRefreshToken(req.headers['refresh-token']);
    spotifyApi
      .createPlaylist(req.query.name, {
        description: 'This playlist was generated by Timbre.',
        public: true,
      })
      .then(
        function (data) {
          res.json({ id: data.body.id });
        },
        function (err) {
          console.log(err);
        }
  ...
  .get('/addtoplaylist', (req, res) => {
    ...
    spotifyApi.addTracksToPlaylist(playlistId, [uri]).then(
      function (data) {
        console.log(data);
